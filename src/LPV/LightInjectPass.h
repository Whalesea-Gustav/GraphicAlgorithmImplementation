...
public:
    ...
    virtual void initV()
    {
        ...
        //着色器编译器会进行自动优化
          ...
          //Init Shader;
        m_pShader = std::make_shared<Shader>("../../../asset/LPV/LightInjectPass_vs.glsl",
                                             "../../../asset/LPV/LightInjectPass_fs.glsl");
...
//        glFramebufferTexture3D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_3D, m_GridG, 0, m_Dimensions.z);
        ...
        glGenTextures(1, &m_GridB);
        glBindTexture(GL_TEXTURE_3D, m_GridB);
        glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA16F, m_Dimensions.x, m_Dimensions.y, m_Dimensions.z, 0, GL_RGBA, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    ...
    virtual void updateV()
    {
        ...
        m_pShader->use();
        glBindImageTexture(0, m_GridR, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA16F);
        glBindImageTexture(1, m_GridG, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA16F);
        glBindImageTexture(2, m_GridB, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA16F);
