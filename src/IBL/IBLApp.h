...
int IBLApp::nRows = 7;
int IBLApp::nColumns = 7;
float IBLApp::spacing = 2.5f;

void IBLApp::initialize()
{
    ...
    this->m_shaders.emplace_back("../../../asset/IBL/precompute_vs.glsl", "../../../asset/IBL/precompute_fs.glsl");
    ...
    iblShader.setVec3("albedo", 0.8f, 0.8f, 0.8f);
...
void IBLApp::frameIBL()
{
    ...
    for (int row = 0; row < IBLApp::nRows; ++row)
    {
        iblShader.setFloat("metallic", (float) row / (float) IBLApp::nRows);
        for (int col = 0; col < IBLApp::nColumns; col++)
        {
            iblShader.setFloat("roughness", glm::clamp((float)col / (float)IBLApp::nColumns, 0.05f, 1.0f));

            glm::vec3 translation((col - (IBLApp::nColumns / 2)) * IBLApp::spacing,
                                  (row - (IBLApp::nRows / 2)) * IBLApp::spacing,
                                  -2.0f);
...
void IBLApp::frame_IBL_diffuse()
{
    ...
    for (int row = 0; row < IBLApp::nRows; ++row)
    {
        pbrShader.setFloat("metallic", (float) row / (float) IBLApp::nRows);
        for (int col = 0; col < IBLApp::nColumns; col++)
        {
            pbrShader.setFloat("roughness", glm::clamp((float)col / (float)IBLApp::nColumns, 0.05f, 1.0f));
...
void IBLApp::frame_IBL_specular_prefilter()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
void IBLApp::frame_IBL_specular_prefilter_precompute()
{

}

void IBLApp::frame_IBL_Cerberus()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
void IBLApp::frame_Debug()
{
}

unsigned int IBLApp::loadTexture(const char *path)
{
